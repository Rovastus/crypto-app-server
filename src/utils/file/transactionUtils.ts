...
export const processTransaction = async function processTransaction(
	...
	let buyWallet: undefined | WalletI = getWalletIfCoinNotFiat(processData.wallets, transactionJsonData.buyCoin);
	let priceWallet: undefined | WalletI = getWalletIfCoinNotFiat(processData.wallets, transactionJsonData.priceCoin);
	let feeWallet: undefined | WalletI = getWalletIfCoinNotFiat(processData.wallets, transactionJsonData.feeCoin);
...
async function pushTransactionBuyTaxEvent(
	...
	let expensesInFiat = transaction.price.mul(priceWallet.avcoFiatPerUnit).plus(feeInFiat);
